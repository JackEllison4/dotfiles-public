...
Item {
    ...
    readonly property string scriptsDir: Quickshell.env("HOME") + "/.config/quickshell/calendar-full"
    ...
    Process {
        id: weatherPoller
        command: ["bash", window.scriptsDir + "/weather-location.sh", "--json"]
        running: true
    ...
    // SCHEDULE DATA
    // -------------------------------------------------------------------------
    property var scheduleData: { "events": [] }

    Process {
        id: schedulePoller
        command: ["bash", window.scriptsDir + "/get-calendar-events.sh", "--json"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let txt = this.text.trim();
                if (txt !== "") {
                    try { window.scheduleData = JSON.parse(txt); } catch(e) { console.log("Calendar Events Parse Error:", e); }
                }
            }
        }
    }
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                            ...
                            RowLayout {
                                anchors.centerIn: parent
                                spacing: window.s(6)
                        Text { text: "Web"; font.family: "JetBrains Mono"; font.weight: Font.Bold; font.pixelSize: window.s(14); color: schLinkMa.containsMouse ? window.base : window.text; visible: false }
                    ...
                    Item {
                        ...
                        ScrollView {
                            ...
                            Column {
                                ...
                                Repeater {
                                    model: window.scheduleData ? window.scheduleData.events : []
                                    ...
                                    delegate: Rectangle {
                                        ...
                                        Column {
                                            ...
                                            Text {
                                                text: modelData.summary || "Untitled"
                                                font.family: "JetBrains Mono"
                                                font.weight: Font.Bold
                                                font.pixelSize: window.s(14)
                                                color: window.text
                                        ...
                                        MouseArea {
                                            anchors.fill: parent
                                            hoverEnabled: true
                                            onEntered: parent.color = Qt.alpha(window.surface0, 0.85)
                                            onExited: parent.color = Qt.alpha(window.surface0, 0.6)
                                        }
                                    }
                                }
                            }
